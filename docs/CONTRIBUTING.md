...
Do **not** manually edit version numbers in `project.pbxproj`. The [version bump workflow](https://github.com/Jubblin/Foghorn/blob/main/.github/workflows/version-bump.yml) handles this on PRs:
...
PRs run [CI](https://github.com/Jubblin/Foghorn/blob/main/.github/workflows/ci.yml) on `macos-26`: SwiftLint + ShellCheck (`quality` job), unit tests, UI smoke tests, then a Release build.
...
GitHub Releases are created when you push a `v*` tag. The release body is taken from the matching `CHANGELOG.md` section (see [release workflow](https://github.com/Jubblin/Foghorn/blob/main/.github/workflows/release.yml)).
